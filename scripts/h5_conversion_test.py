...
class ConvertH5WeightsTest(unittest.TestCase):
  ...
  def testConvertWeightsFromSimpleModelTFJS(self):
    ...
    np.allclose(np.ones([4, 2]), kernel2['data'])
  ...
  def testConvertWeightsFromSequentialModelTFJS(self):
    ...
    np.allclose(np.ones([2, 3]).tolist(), kernel1['data'])
    bias1 = weights1[1]
    self.assertEqual('Dense10/bias:0', bias1['name'])
    self.assertEqual('float32', bias1['data'].dtype)
    self.assertEqual((3, ), bias1['data'].shape)
    np.allclose(np.zeros([3]).tolist(), bias1['data'])
    weights2 = groups[1]
    self.assertEqual(1, len(weights2))
    kernel2 = weights2[0]
    ...
    np.allclose(np.ones([3, 1]).tolist(), kernel2['data'])
