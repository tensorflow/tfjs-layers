...
export class DepthwiseConv2D extends Conv2D {
  ...
  build(inputShape: Shape|Shape[]): void {
    ...
    const channelAxis = this.dataFormat === DataFormat.CHANNEL_FIRST ? 1 : 3;
  ...
  computeOutputShape(inputShape: Shape|Shape[]): Shape|Shape[] {
    inputShape = getExactlyOneShape(inputShape);
    const rows = this.dataFormat === DataFormat.CHANNEL_FIRST ? inputShape[2] :
                                                                inputShape[1];
    const cols = this.dataFormat === DataFormat.CHANNEL_FIRST ? inputShape[3] :
                                                                inputShape[2];
    const outFilters = this.dataFormat === DataFormat.CHANNEL_FIRST ?
    ...
    if (this.dataFormat === DataFormat.CHANNEL_FIRST) {
      ...
      // In this case, assume DataFormat.CHANNEL_LAST.
