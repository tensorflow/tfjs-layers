     ...
     async done => {
       ...
       for (let n = 0; n < 2; ++n) {
         ...
         for (let epochIndex = 0; epochIndex < epochs; ++epochIndex) {
           ...
           // N.B.: Across epochs, the tensor count will increase, due to the
           // loss and metric values stored for the returned history object,
           // which are currently downloaded via data() calls at the end of the
           // fit() call.
           const inEpochTensorCounts = tensorCounts.slice(
               batchesPerEpoch * epochIndex,
               batchesPerEpoch * epochIndex + batchesPerEpoch - 1);
