...
export class Model extends Container implements tfc.InferenceModel {
   ...
   * @param iteratorOut
   */
  protected checkDataIteratorOutput(iteratorOut: TensorContainer): Tensor[] {
    tfc.util.assert(
    ...
    if (xs instanceof tfc.Tensor) {
      tfc.util.assert(
          ...
          `Model has multiple ${this.inputs.length} inputs, hence it ` +
              `expects the input dataset to generate a dictionary of tensors ` +
              ` (with keys ${JSON.stringify(this.inputNames)}, ` +
              `but received a single tensor.`);
      return [xs, ys];
    } else {
      xs = xs as TensorMap;
      const flattendXs: Tensor[] = [];
